...
class EyegazeClassifier(object):
    ...
    def preproc(
        ...
        if median_filter_length:
            ...
            med_velocities = np.zeros((len(data['x']),), velocities.dtype)
            med_velocities[1:] = (
    ...
    def draw_fixations(self, events):
        # create a figure
        fig = plt.figure(figsize=(200, 300), dpi=100, frameon=False)
    ...
    def parse_fixations(self, events):
        fixations = {'x': [], 'y': [], 'duration': []}
        for e in events:
            if e['label'] == 'FIXA':
                fixations['x'].append((e['start_x'] + e['end_x']) / 2)
                fixations['y'].append((e['start_y'] + e['end_y']) / 2)
                fixations['duration'].append(e['end_time'] - e['start_time'])
        return fixations
        ...
        if events is not None:
            for ev in events:
                pl.axvspan(
                    ...
                    alpha=0.8)
        ntimepoints = len(pp) if pp is not None else len(data['x'])
